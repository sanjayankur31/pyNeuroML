...
def run_lems_with_jneuroml_brian2(lems_file_name,
                                  paths_to_include = [], 
                                  max_memory = DEFAULTS['default_java_max_memory'], 
                                  skip_run = False,
                                  nogui = False, 
    ...
    print_comment("Loading LEMS file: %s and running with jNeuroML_Brian2" \
                  % lems_file_name, verbose)
...
def get_path_to_jnml_jar():
    ...
    jar_path = os.path.abspath("/usr/share/java/jNeuroML-%s.jar" % JNEUROML_VERSION)
